...
def save_audio(yt: YouTube, filename_base: str, folder: str):
    """Save audio of youtube."""
    stream = yt.streams.filter(only_audio=True).order_by("abr").last()
    ext = stream.mime_type.split("/")[-1]
    filename_mp3 = filename_base + ".mp3"
    filename_down = filename_base + f".{ext}"
    path_down = osp.join(folder, filename_down)
    stream.download(output_path=folder, filename=filename_down)
...
def save_video(yt: YouTube, filename_base: str, folder: str):
    """Save audio of youtube."""
    stream = yt.streams.filter(mime_type="video/mp4", progressive=True).order_by(
        "resolution"
    )[-1]
    filename_mp4 = filename_base + ".mp4"
    stream.download(output_path=folder, filename=filename_mp4)


def download_youtube_link(
    link: str, only_video_flag: bool, only_audio_flag: bool, folder: str
):
    """Start the download operation."""
    yt = YouTube(link)

    filename_base = title2filename(yt.title)
    path_base = osp.join(folder, filename_base)
    save_caption(yt, path_base)

    if only_video_flag:
        save_video(yt, filename_base, folder)
    elif only_audio_flag:
        save_audio(yt, filename_base, folder)
    else:
        save_audio(yt, filename_base, folder)
        save_video(yt, filename_base, folder)
